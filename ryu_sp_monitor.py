# 运行在RYU控制器，继承于ryu_sp_route.ShortestPath,使用最短路算法对数据包进行路由并给交换机添加流表，
# 定时记录网络负载，将网络负载记录写入sp_net_load.txt文件，并画出图
...
class MyMonitor(ryu_sp_route.ShortestPath):
    ...
    def _monitor(self):
        ...
        while True:
            ...
            if len(self.net_load)>=monitor_time/monitor_period:
                ...
                plt.title("SP network load")
                plt.savefig('load_of_ShortestPath.png')
                # plt.show()
                with open('sp_net_load.txt','wb') as f:
                ...
                print("topo name\tSFC length\tSFC number\tSFC BandWidth\tOSPF Resource\tOSPF Success Rate\n")
                print(setting.topo,"\t",'[' + str(setting.sfc_size_range[0]) + ', '+ str(setting.sfc_size_range[1]) + ']',"\t   ", setting.num_of_sfc,"\t\t",\
                                    '[' + str(setting.sfc_link_range[0]) + ', '+ str(setting.sfc_link_range[1]) + ']', "\t", "  %.2f%%"%(sum(self.net_load[-10:]) / 10 * 100), "\t", "   %.2f%%"%(success_rate * 100))
                print("\n-------------------------------------------------------------------------------------------------")
    ...
    def _port_status_handler(self, ev):
        ...
        if(reason==ofproto.OFPPR_DELETE and self.has_plot==False):#拓扑结束
            ...
            plt.savefig('load of ShortestPath.png')
            #plt.show()
